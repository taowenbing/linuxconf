...
# - Create a set of 2 CDROM ISO images
...
# for instance mirror Slackware 13.0 by passing '-r 13.0' to the script.
...
# Set ISO="CDROM" if you want two CD ISO's instead of a single DVD.
...
do
  case $Option in
	...
	EOH
        ...
        echo "                CDROM: produce 2 CDROM images;"
...
_EOT_
  ...
  #
  # Slackware installation uses 2 CD's.
  # In 'menu' or 'expert' mode, packages from CD2 can be selected.
  # In 'full install' mode, additional CD's will be asked for.
  #
  # CD1: bootable INSTALL CD
  # CD2: k, kde, x, xap
  #
  # Command used to create the ISO's for CD1 and CD2:
  # (see also /isolinux/README.TXT on the CDROM you'll burn from the ISO)
...
_EOT_
  ...
  echo "$(date) [$$]: First CDROM ISO created (exit code ${MKISOERR}) - one to go ..."
  ...
  if [ -f ${SLACKROOTDIR}/${SLACKRELEASE}-iso/${SLACKRELEASE}-install3.iso ];
  then
    # Deleting obsolete CDROM ISO #3 - it's down to two ISOs now:
